...
from connect import parse_post_request, execute_request
...
def edit_ingredient_description(id):
	...
	if type(json) == dict:
		...
		#ingredient_id = json.get("id", None)
		name = json.get("name", None)
		is_allergen = json.get("is_allergen", None)
		measure_id = json.get("measure_id", None)
		ingredient_categories_id = json.get("ingredient_categories_id", None)
...
def add_initial_ingredients_in_dish(id):
	...
	if type(json) == dict:
		...
		#ingredients_amounts = json.get("ingredients_amounts")
...
def edit_dish_description(id):
	...
	if type(json) == dict:
		...
		if search_for_errors(errors) is None:

			if price is not None:

				price = round(price, 2)

			if ingredients_amounts is not None:

				ingredients_result = execute_request(q.update_rows, "list", 200, query_params = ["ingredients_in_dishes", "amount_per_serving", "dish_id", "ingredient_id", ingredients_amounts, id])
				...
				result = execute_request(q.update_row, "index", 200, query_params = ["dishes", "id", json, id])[0]
...
def get_ingredient_shipments(id):
	...
	if error is None:
		...
		if check_old_values_true(get_old_values_from(None, "ingredient_id", custom_query = q.current_ingredient_amounts))(id) is None:
			...
			if json.get("shipments", None) is not None:

				for i in json["shipments"]:

					if json["shipments"][i]["due_date"] is not None:

						if d.date.today() < json["shipments"][i]["due_date"] or json["shipments"][i]["current_amount"] == 0:
							json["shipments"][i]["should_remain_active"] = True
						else:
							json["shipments"][i]["should_remain_active"] = False
					else:
						json["shipments"][i]["should_remain_active"] = True
